        ...
        {items.length === 0 ? (
          ...
          <ul className="divide-y divide-gray-700">
            {items.map(item => (
              <li key={item.id}>
                <button
                  className={`w-full text-left p-3 hover:bg-gray-700/50 ${selectedId === item.id ? 'bg-gray-700/70' : ''}`}
                ...
                >
                  ...
                  {item.answer.sql && (
                    <div className="mt-1 text-[11px] text-cyan-300 font-mono truncate" title={item.answer.sql || ''}>
                      {item.answer.sql}
                    </div>
                  )}
                </button>
